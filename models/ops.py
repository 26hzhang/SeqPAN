...
def gumbel_sample(shape):
    g_sample = tf.random_uniform(shape, minval=0, maxval=1)
    noise = -tf.log(-tf.log(g_sample + 1e-20) + 1e-20)
    return noise
...
def gumbel_softmax(logits, tau, hard=False):
    ...
    g_sample = tf.random_uniform(tf.shape(logits), minval=0, maxval=1)
    noise = -tf.log(-tf.log(g_sample + 1e-20) + 1e-20)
    # compute gumbel softmax
    y = tf.nn.softmax((logits + noise) / tau)
    if hard:
        y_hard = tf.cast(tf.equal(y, tf.reduce_max(y, 1, keepdims=True)), dtype=y.dtype)
...
def gumbel_sigmoid(logits, tau, hard=False):
    ...
    g_sample1 = tf.random_uniform(tf.shape(logits), minval=0, maxval=1)
    g_sample2 = tf.random_uniform(tf.shape(logits), minval=0, maxval=1)
    noise = -tf.log(tf.log(g_sample2 + 1e-20) / tf.log(g_sample1 + 1e-20) + 1e-20)
...
def label_smoothing(labels, mask, epsilon=0.1):
    ...
    seq_len = tf.reduce_sum(mask, axis=1, keepdims=False)  # (batch_size)
...
def create_attention_mask(from_mask, to_mask, broadcast_ones=False):
    ...
    to_mask = tf.cast(tf.expand_dims(to_mask, dim=1), dtype=tf.float32)
    if broadcast_ones:
        mask = tf.ones(shape=[batch_size, from_seq_len, 1], dtype=tf.float32)
    else:
        mask = tf.cast(tf.expand_dims(from_mask, dim=2), dtype=tf.float32)
...
def trilinear_attention(args, maxlen1, maxlen2, drop_rate=0.0, reuse=None, name='efficient_trilinear'):
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        weights4arg1 = tf.get_variable('linear_kernel4arg1', [arg_size, 1], dtype=dtype)
        weights4mlu = tf.get_variable('linear_kernel4mul', [1, 1, arg_size], dtype=dtype)
        # compute results
        ...
        subres1 = tf.tile(tf.transpose(tf.matmul(drop_args[1], weights4arg1), perm=(0, 2, 1)), [1, maxlen1, 1])
